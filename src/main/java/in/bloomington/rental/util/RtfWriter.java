...
public class RtfWriter{
    ...
    boolean debug = false;
		...
		String varianceText = "";
		int unitCount = 0, structureCount = 0, occupLoadCount = 0;
		String occupLoad = "", storyCount="";
		String inspectionType = "", addressStr="", heatSource="",
				foundation="", storyCnt = ""; //for multiple buildings
		// Hashtable<String, String> inspMap = null;
		Font font, fonts, fontb, fontbb, fontlb;
		//
		// we need one for egress info
		RentalStructure structure = null;
		public RtfWriter(Rental val,
										 Inspection val2,
										 String val3,
										 String val4){
				setRent(val);
            ...
            RtfWriter2 writer = RtfWriter2.getInstance(document, new FileOutputStream(fileAndPath));
						...
						if(inspectionType.startsWith("Cycle")){
								if(structure != null && structure.hasEgressInfo()){
										...
										if(str != null)
												egressDecreeYears = str;
										str = " Monroe County Assessor records indicate that this structure was built in "+yearBuilt+". These are the minimum requirements for emergency egress for structures built between "+egressDecreeYears+":\n";
