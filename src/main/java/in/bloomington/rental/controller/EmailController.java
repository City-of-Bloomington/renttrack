...
public class EmailController
{
    ...
    public String emailLogDetails(@PathVariable("id") int id, Model model)
    {
        if (message != null) model.addAttribute("message", message);
    ...
    List<Owner> checkInvalidEmails(String dateFrom, String dateTo)
    {
        ...
        if (map.size() > 0) {
            for (Owner owner : map.keySet()) {
                    ...
                    if (emails != null && emails.size() > 0) {
                        for (String email : emails) {
                            if (to.equals("")) {
                                ...
                                if (to.indexOf(email) == -1 && bcc.indexOf(email) == -1) {
                                    if (!bcc.equals("")) {
                                        bcc += ",";
                                    }
                                    bcc += email;
                ...
                for (Rental rental : set) {
                            ...
                            if (emails != null && emails.size() > 0) {
                                for (String email : emails) {
                                    if (to.equals("")) {
                                        ...
                                        if (bcc.indexOf(email) == -1 && to.indexOf(email) == -1) {
                                            if (!bcc.equals("")) {
                                                bcc += ",";
                                            }
                                            bcc += email;
                    ...
                    if (addresses != null) {
                        for (Address addr : addresses) {
                            emailText += "ID: " + rental.getId() + ", Expires: " + rental.getPermitExpiresFr() + ", "
                ...
                if (fileName != null && !fileName.equals("")) {
                    EmailHandle ehl = new EmailHandle(to, emailer.getEmailFrom(), emailer.getSubject(), emailText, cc,
                        bcc);
                    ...
                    EmailHandle ehl = new EmailHandle(to, emailer.getEmailFrom(), emailer.getSubject(), emailText, cc,
                        bcc, fileName, filePath);
                    System.err.println(ehl);
                    // error = ehl.sendWAttach();
                }
                EmailDetailLog eml = new EmailDetailLog(emailLog, to, cc, bcc, ownerIds, agentIds, rentalIds, error);
                emailDetailLogService.save(eml);
                /*
                 * System.err.println(" to "+to); System.err.println(" bcc "+bcc);
