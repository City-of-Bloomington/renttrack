...
public class AddressController
{
    ...
    public String updateAddress(@ModelAttribute("address") @Valid Address address, BindingResult result, Model model)
    {
        if (result.hasErrors()) {
    ...
    public String popFindAddress(@ModelAttribute("address") @Valid Address address, BindingResult result, Model model)
    {
        if (result.hasErrors()) {
            return "popAddress";
        }
    ...
    public String savePopAddress(@ModelAttribute("address") @Valid Address address, BindingResult result, Model model)
    {
        if (result.hasErrors()) {
    ...
    public String editPopAddress(@PathVariable("id") int id, Model model)
    {
        Address address = addressService.get(id);
        model.addAttribute("address", address);
        return "popAddress";
    ...
    public String saveAddress(@ModelAttribute("address") @Valid Address address, BindingResult result, Model model)
    {
        if (result.hasErrors()) {
...
class AddressServiceController
{
    ...
    private String buildJson(List<Item> ones)
    {
        ...
        for (Item one : ones) {
            // if(!json.equals("")) json += ",";
            // json +=
            // "{\"id\":"+one.getId()+",\"value\":"+JSONObject.quote(one.getName())+"}";
