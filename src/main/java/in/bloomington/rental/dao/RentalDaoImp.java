...
public class RentalDaoImp implements RentalDao{
    ...
    public void delete(int id) {
        Rental rental = (Rental) sessionFactory.getCurrentSession().load(
                Rental.class, id);
        if (rental != null) {
        ...
        Rental rental = sessionFactory.getCurrentSession().get(Rental.class, id);
		...
		public List<Rental> search(Search search){
				...
				if(id != null && id > 0){
						...
						qq += ", rental_owners ro where ro.rental_id=r.id and ro.owner_id="+ownerId;
				}
				else if(addressId != null && addressId > 0){
						qq = "select r.* from rentals r, "; 		
						qq +=	"rental_structures s, rental_units u "+
								" where s.rental_id=r.id "+
								" and u.structure_id=s.id "+
