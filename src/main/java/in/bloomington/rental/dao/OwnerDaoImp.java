...
public class OwnerDaoImp implements OwnerDao{
    ...
    public void delete(int id) {
        Owner owner = (Owner) sessionFactory.getCurrentSession().load(
                Owner.class, id);
        if (owner != null) {
    ...
    public void removeRentalOwner(int id) {
        RentalOwner val = (RentalOwner) sessionFactory.getCurrentSession().load(RentalOwner.class, id);
        if (val != null) {
		...
		public List<Owner> search(Owner owner){
				...
				String qw = "";
				String name=null,address=null,city=null,state=null,
						zip=null, phone=null, email=null;
				...
				if(ownerId  != null && ownerId > 0){
						...
						if(phone != null && !phone.equals("")){
								qq = "select o.* from owners o,owner_phones p where p.owner_id=o.id and p.phone_num like '%"+phone+"%' order by o.name ";
						}
						else {
		...
		/**
   select o.id,o.name,r.id,r.permit_expires,o.email from owners o,rental_owners p, rentals r where p.owner_id=o.id and p.rental_id=r.id  and r.permit_expires >= to_date('01/01/2018','MM/DD/YYYY') and r.permit_expires <= to_date('02/01/2018','MM/DD/YYYY') and r.inactive is null and o.email is not null order by o.name limit 5 ";
		...
		public List<Owner> findOwnersForExpireEmail(String startDate,
																					 String endDate){
				Session session = sessionFactory.getCurrentSession();
				String qq = "select o.* from owners o,rental_owners p, rentals r where p.owner_id=o.id and p.rental_id=r.id  and r.permit_expires >= to_date('"+startDate+"','MM/DD/YYYY') and r.permit_expires <= to_date('"+endDate+"','MM/DD/YYYY') and r.inactive is null and o.email is not null order by o.name ";
				SQLQuery query = session.createSQLQuery(qq);
				query.addEntity("owner", Owner.class);
				List<Owner> owners = query.list();		
				return owners;
