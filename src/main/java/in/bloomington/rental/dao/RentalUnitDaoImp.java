...
public class RentalUnitDaoImp implements RentalUnitDao{
    ...
    public void update(int id, RentalUnit rentalUnit) {
      ...
      RentalUnit rentalUnit2 = session.byId(RentalUnit.class).load(id);
			rentalUnit2 = (RentalUnit)session.merge(rentalUnit);
			session.update(rentalUnit2); 
      session.flush();				
    }
		@Override
    ...
    public void delete(int id) {
        RentalUnit rentalUnit = (RentalUnit) sessionFactory.getCurrentSession().load(
                RentalUnit.class, id);
        if (rentalUnit != null) {
            this.sessionFactory.getCurrentSession().delete(rentalUnit);
        }
    }
		@Override		
